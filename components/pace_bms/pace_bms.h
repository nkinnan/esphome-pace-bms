...
class PaceBms : public PollingComponent, public uart::UARTDevice {
 public:
  ...
  void register_cell_over_voltage_configuration_callback(std::function<void(PaceBmsV25::CellOverVoltageConfiguration&) > callback) { ESP_LOGE(TAG, "cell over voltage pointer value: %i", callback); cell_over_voltage_configuration_callbacks_.push_back(std::move(callback)); }
