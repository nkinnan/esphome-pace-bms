...
void PaceBms::update() {
    ...
    //}
    //else {
    ESP_LOGV(TAG, "***********Requesting analog information flow control %s, address %i", this->flow_control_pin_->dump_summary().c_str(), this->address_);
...
void PaceBms::dump_config() {
  ...
  if (this->flow_control_pin_ != nullptr) {
    ...
    ESP_LOGCONFIG(TAG, "  Flow Control Pin: N/A");
  }
