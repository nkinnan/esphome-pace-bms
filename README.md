...
pace_bms:
  ...
  rx_buffer_size: 256
```
Most of these settings are only applicable if you have a single battery pack (or if you have one ESP per pack).  That would be considered a `type=MASTER` (or with type omitted since MASTER is the default value) BMS.  To configure a slave BMS in a multi-pack setup, see [multi-pack configuration](#Support-for-multiple-battery-packs).  
...
* **request_throttle:** Minimum interval between sending requests to the BMS.  Increasing this may help if your BMS "locks up" after a while, it's probably getting overwhelmed.  Increasing this can also help in a multi-pack setup by giving the master BMS enough time to ask all the slaves for their information.  See [multi-pack configuration](#Support-for-multiple-battery-packs) for more information on that.
